...
#this script takes a bedtools merged file and a list of samples that were merged
#the merge file should be like this:
#chrom	start	stop	names(comma delimited)	scores(comma delimited)	gene names(comma delimited if multiple)	distance to gene
#it outputs a matrix with a header
#the first column is a unique identifier: chr_start_stop_gene_dist
#run like this:  python global_matrix_gene_dist.py merged_bed_data.merge merged_sample_list.txt
...
def main():
    ...
    for line in masterPeakFile:
		...
		#version for gene name:
        identifier = masterLineList[0] + ':' + masterLineList[1] + '_' + masterLineList[2] + '_' + string.strip(masterLineList[5]) + '_' + string.strip(masterLineList[6]) # 6th column should be gene name, 7th should be distance to that gene
    ...
    for i,item in enumerate(rowList): #each item in rowList should be a list
        ...
        for i,elem in enumerate(item):
            if len(item) - i == 1: #should be the last column in a line
                if elem == 0.0:
                    matrixFile.write(' ')#hence the whitespace!!
                else:               
